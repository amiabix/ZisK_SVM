...
pub fn create_corrected_memory_program() -> Result<Vec<u8>> {
    ...
    program.extend_from_slice(&[
        ...
        BPF_LD_IMM64, 0x30, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // First part (dst=r3, src=r0)
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01,         // Second part (high 32 bits)
        
        // MOV r1, 0x12345678 (test value)
        BPF_ALU64_MOV_IMM, 0x10, 0x00, 0x00, 0x78, 0x56, 0x34, 0x12, // dst=r1, src=r0
        
        // Store r1 at heap address: STX [r3 + 0], r1
        BPF_STX_DW, 0x31, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // dst=r3, src=r1
        
        // Load back from heap: LDX r2, [r3 + 0]
        BPF_LDX_DW, 0x23, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // dst=r2, src=r3
        
        // Compare loaded value with original: JNE r2, r1, +3
        BPF_JMP_JNE_REG, 0x21, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00, // dst=r2, src=r1
        
        // Success: MOV r0, 1
        BPF_ALU64_MOV_IMM, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, // dst=r0, src=r0
        
        // Jump to exit: JA +2
        BPF_JMP_JA, 0x00, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00,
        
        // Failure: MOV r0, 0
        BPF_ALU64_MOV_IMM, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // dst=r0, src=r0
